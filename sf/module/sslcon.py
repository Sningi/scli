        ...
        if filter == "all":
            ...
            if type(passthrough_ip_list) == list:
                for ip in passthrough_ip_list:
                    clean_data.append(
                        {"op": "remove", "path": "/passthrough_ip_list", "value": ip})
            if len(clean_data) > 0:
                data = hp.cpu_patch(
...
def sslcon_server_config_add(info,
    ...
    files = []
    if key_file:
        files.append(("key_file", (key_file.name.split(
            "/")[-1], key_file.read(), "application/octet-stream")))

    if cert_file:
        files.append(("cert_file", (cert_file.name.split(
            "/")[-1], cert_file.read(), "application/octet-stream")))

    if chain_file:
        files.append(("chain_file", (chain_file.name.split(
            "/")[-1], chain_file.read(), "application/octet-stream")))
