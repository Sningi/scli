...
def intf_sw(op, intf, filter=None):
    ...
    if op == 'show':
        ...
        """
        data = []
        op_data = [{"op": "add", "path": "/configuration/speed", "value":speed_comp[filter][-1]}]
        for idx in restid:
        ...
        data = []
        op_data = [{"op": "add", "path": "/configuration/transceiver_mode", "value":filter}]
        for idx in restid:
            temp = hp.sw_patch('interfaces/{0}'.format(idx),op_data)
            for i in temp:
                i[1]+= " "+idx
        ...
        data = []
        op_data = [{"op": "add", "path": "/configuration/transceiver_mode", "value":"rx"}]
        for idx in restid:
            temp = hp.sw_patch('interfaces/{0}'.format(idx),op_data)
            for i in temp:
                i[1]+= " "+idx
