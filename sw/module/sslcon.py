...
def sslcon_sw_base_configuration_create():
    ...
    interface_base_data = [
            ...
            {"op":"add", "path":"/configuration/attach_openflow_instance", "value":SSLCON_OPENFLOW_INSTANCE_NAME},
            {"op":"add", "path":"/configuration/vlan_tag", "value":SSLCON_BASE_VLAN}
    ]
    tb = None
    for intf in range(1, 55):
        tmp = deepcopy(interface_base_data)
        tmp.append({"op":"add", "path":"/configuration/ofport", "value":str(intf)})
        if intf == 54:
            tmp.append( {"op":"add", "path":"/configuration/vlan_mode", "value":"trunks"})
            tmp.append( {"op":"add", "path":"/configuration/vlan_trunks", "value":SSLCON_MAP_PORT_VLAN})
        else:
            tmp.append( {"op":"add", "path":"/configuration/vlan_mode", "value":"access"})
