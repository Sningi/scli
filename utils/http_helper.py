...
class Http:
    ...
    async def login_may_use_cookie(self, clear_cookie=False, diff_time=7190):# diff_time 2h-10s
        ...
        if os.path.isfile(cookie) and time.time()- os.path.getmtime(cookie)< 7190:
            ...
            with_cookie = True
        if not clear_cookie:
            self.session = aiohttp.ClientSession(connector=TCPConnector(ssl=False),cookie_jar=jar)
...
class Helper:
    ...
    def __init__(self, cfg):
        ...
        tasks = [self.loop.create_task(sw.login_may_use_cookie()) for sw in self.sws]
        tasks += [self.loop.create_task(cpu.login_may_use_cookie()) for cpu in self.cpus]
        wait_login = asyncio.wait(tasks)
        self.loop.run_until_complete(wait_login)

    def cpu_get(self, url, data = None, params = None):
        tasks = [self.loop.create_task(rest.get(url, data, params)) for rest in self.cpus]
        get = asyncio.wait(tasks)
        self.loop.run_until_complete(get)
        return [task.result() for task in tasks]

    def cpu_put(self, url, data = None, params = None):
        tasks = [self.loop.create_task(rest.put(url, data, params)) for rest in self.cpus]
        put = asyncio.wait(tasks)
        self.loop.run_until_complete(put)
        return [task.result() for task in tasks]

    def cpu_post(self, url, data = None, params = None):
        tasks = [self.loop.create_task(rest.post(url, data, params)) for rest in self.cpus]
    ...
    def cpu_raw_post(self, url, data = None, header = None, params = None, files = None):
        tasks = [self.loop.create_task(rest.raw_post(url, data, header, params, files)) for rest in self.cpus]
        post = asyncio.wait(tasks)
        self.loop.run_until_complete(post)
        return [task.result() for task in tasks]

    def cpu_patch(self, url, data = None, params = None):
        tasks = [self.loop.create_task(rest.patch(url, data, params)) for rest in self.cpus]
        patch = asyncio.wait(tasks)
        self.loop.run_until_complete(patch)
        return [task.result() for task in tasks]

    def cpu_delete(self, url, data = None, params = None):
        tasks = [self.loop.create_task(rest.delete(url, data, params)) for rest in self.cpus]
        delete = asyncio.wait(tasks)
        self.loop.run_until_complete(delete)
        return [task.result() for task in tasks]

    def sw_get(self, url, data = None, params = None):
        tasks = [self.loop.create_task(rest.get(url, data, params)) for rest in self.sws]
        get = asyncio.wait(tasks)
        self.loop.run_until_complete(get)
        return [task.result() for task in tasks]

    def sw_put(self, url, data = None, params = None):
        tasks = [self.loop.create_task(rest.put(url, data, params)) for rest in self.sws]
        put = asyncio.wait(tasks)
        self.loop.run_until_complete(put)
        return [task.result() for task in tasks]

    def sw_post(self, url, data = None, params = None):
        tasks = [self.loop.create_task(rest.post(url, data, params)) for rest in self.sws]
        post = asyncio.wait(tasks)
        self.loop.run_until_complete(post)
        return [task.result() for task in tasks]

    def sw_raw_post(self, url, data = None, header = None, params = None, files = None):
        tasks = [self.loop.create_task(rest.raw_post(url, data, header, params, files)) for rest in self.sws]
        post = asyncio.wait(tasks)
        self.loop.run_until_complete(post)
        return [task.result() for task in tasks]

    def sw_patch(self, url, data = None, params = None):
        tasks = [self.loop.create_task(rest.patch(url, data, params)) for rest in self.sws]
        patch = asyncio.wait(tasks)
        self.loop.run_until_complete(patch)
        return [task.result() for task in tasks]

    def sw_delete(self, url, data = None, params = None):
        tasks = [self.loop.create_task(rest.delete(url, data, params)) for rest in self.sws]
        delete = asyncio.wait(tasks)
        self.loop.run_until_complete(delete)
        return [task.result() for task in tasks]
    ...
    def __del__(self):
        ...
        tasks += [self.loop.create_task(cpu.del_session()) for cpu in self.cpus]
        wait_login = asyncio.wait(tasks)
        self.loop.run_until_complete(wait_login)
        self.loop.close()
